    ...
    DefaultFlickable {
        ...
        ColumnLayout {
            ...
            FAQLine {
                ...
                text: qsTr("No! SmartFi is non-custodial. We never store any sensitive data, including your private keys, seed phrases, or PIN. This data is  only stored on the user’s device and never leaves it. You are in full control of your assets.")
            }

            FAQLine {
                title: qsTr("How is trading on SmartFi different from trading on other DEXs?")
...
SmartFi enables you to natively trade across two different blockchain networks without proxy tokens. You can also place multiple orders with the same funds. For example, you can sell 0.1 BTC for KMD, QTUM, or VRSC — the first order that fills automatically cancels all other orders.")
            }

            FAQLine {
                title: qsTr("How long does each atomic swap take?")
                text: qsTr('Several factors determine the processing time for each swap. The block time of the traded assets depends on each network (Bitcoin typically being the slowest) Additionally, the user can customize security preferences. For example,  (you can ask SmartFi to consider a KMD transaction as final after just 3 confirmations which makes the swap time shorter compared to waiting for a <a href="https://komodoplatform.com/security-delayed-proof-of-work-dpow/">notarization</a>.')
            }
            ...
            FAQLine {
                title: qsTr("How are the fees on SmartFi calculated?")
                text: qsTr("There are two fee categories to consider when trading on SmartFi.
...
Network fees can vary greatly depending on your selected trading pair.")
            ...
            FAQLine {
                ...
                text: qsTr('Yes! SmartFi offers support through the <a href="https://komodoplatform.com/discord">Komodo Discord server</a>. The team and the community are always happy to help!')
            }

            FAQLine {
                title: qsTr("Who is behind SmartFi?")
                text: qsTr("SmartFi is developed by the Komodo team. Komodo is one of the most established blockchain projects working on innovative solutions like atomic swaps, Delayed Proof of Work, and an interoperable multi-chain architecture.")
            }

            FAQLine {
                title: qsTr("Is it possible to develop my own white-label exchange on SmartFi?")
                text: qsTr("Absolutely! You can read our developer documentation for more details or contact us with your partnership inquiries. Have a specific technical question? The SmartFi developer community is always ready to help!")
            }

            FAQLine {
                title: qsTr("Which devices can I use SmartFi on?")
                text: qsTr('SmartFi is available for mobile on both <a href="https://atomicdex.io/">Android and iPhone, and for desktop on Windows, Mac, and Linux</a> operating systems.')
            }
